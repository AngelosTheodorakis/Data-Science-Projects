...
ggplot(data, aes(x = factor(Gradyear))) +  
  ...
  labs(title='¸ôïò Áðïöïßôçóçò', x='Graduation Year',y='Percent')+
...
p1<-ggplot(data, aes(x = factor(Q1))) +  
  ...
  scale_y_continuous(labels = percent) +
  labs(title='Öýëï', x='Öýëï',y='Ðïóïóôü')+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_x_discrete(labels=c('Ãõíáßêåò','¢íäñåò'))

p2<-ggplot(data, aes(x = factor(Age))) +  
  geom_bar(aes(y = (..count..)/sum(..count..)), fill = "blue") + 
  scale_y_continuous(labels = percent)+
  labs(title='Çëéêßá', x='ÇëéêéáêÞ ïìÜäá',y='Ðïóïóôü')+
  theme(plot.title = element_text(hjust = 0.5))+
...
ggplot(data, aes(x = factor(Q13),fill = factor(Q8))) +  
  ...
  scale_y_continuous(labels = percent)+
  labs(title='ÓõíÜöåéá ÐÌÓ ìå åñãáóßá êáé óýóôáóç ÐÌÓ', x='Èá ðñïôåßíáôå ôï ÐÌÓ;',y='Ðïóïóôü',
       fill='ÓõíÜöåéá ìå åñãáóßá')+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_x_discrete(labels=c('Óßãïõñá íáé','ÌÜëëïí íáé','Ïýôå íáé ïýôå ü÷é','ÌÜëëïí ü÷é','Óßãïõñá ü÷é'))+
scale_fill_manual(values=c("darkred","red","blue","green4","darkgreen","yellow"),
  labels = c("Ðïëý ", "ÁñêåôÜ ", "¼÷é êáé ôüóï ", "Êáèüëïõ ", "Äåí Ý÷ù åñãáóôåß",'NA'))
